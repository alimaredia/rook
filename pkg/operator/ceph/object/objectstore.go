...
func removeObjectStoreFromMultisite(objContext *Context, spec cephv1.ObjectStoreSpec) error {
	...
	if zoneIsMaster {
		...
		logger.Debugf("endpoint %v was removed from zone group %v. the remaining endpoints in the zone group are %v", objContext.Endpoint, objContext.ZoneGroup, zoneEndpoints)
	...
	logger.Debugf("endpoint %v was removed from zone %v. the remaining endpoints in the zone are %v", objContext.Endpoint, objContext.Zone, zoneEndpoints)

	if zoneIsMaster && zoneGroupIsMaster && zoneEndpoints == "" {
		logger.Infof("WARNING: No other zone in realm %v can commit to the period or pull the realm until you create another object-store in zone %v", objContext.Realm, objContext.Zone)
	}
	...
	logger.Infof("period for realm %v successfully updated after removal of object-store %v", objContext.Realm, objContext.Name)
