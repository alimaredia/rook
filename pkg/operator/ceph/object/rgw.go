...
func (c *clusterConfig) deleteStore() error {
	...
	if !c.clusterSpec.External.Enable {
		...
		realmName, zoneGroupName, zoneName, err := GetMultisiteForObjectStore(c.context, c.store)
		if err != nil {
			return errors.Wrapf(err, "failed to get realm/zone group/zone for object store %q", c.store.Name)
		}
